...
class AScriptClient:
    ...
    def element_clear_text(self, node_id: str, x: float, y: float, *, max_cycles: int = 40, log: bool = False, refind: Callable[[], tuple[str, float, float]] | None = None) -> bool:
        """伪清空元素文本：聚焦后循环「双击选中 → 编辑菜单剪切」，直到元素值为空。

        设备端 WDA 的原生 ``/element/:id/clear``、``/value`` 与退格键在该
        实现上均无效（真机确认），因此通过 iOS 编辑菜单逐词剪切模拟清空。
        ``(x, y)`` 为元素中心的截图物理像素坐标。``refind`` 在元素句柄因
        UI 变化失效时重新定位元素（树 UUID 一轮剪切后即过期，必须刷新）。
        返回是否清空；连续两轮值不再缩短时提前失败，避免空转。
        """
        def read_value() -> str:
            value = self.eval_python("from ascript.ios.node import Node\nfrom ascript.ios.system import client as sc\n_result=str(Node(sc, %r).value)" % node_id)
            return value if isinstance(value, str) else str(value)

        def try_refind() -> None:
            nonlocal node_id, x, y
            if refind is None: return
            try:
                fresh = refind()
            except (DeviceOperationError, DeviceResponseError, DeviceConnectionError):
                return
        ...
        stale_rounds, previous, failed_reads, missing_menus = 0, None, 0, 0
        for cycle in range(1, max_cycles + 1):
            # 先双击选择并剪切,再读取值校验:读取用的设备端 WDA GET 会干扰
            # 紧随其后的 HID 双击,导致编辑菜单不弹出(真机实测)。
            self.double_tap(x, y)
            time.sleep(0.8)
            items = self.find_elements({"sel": [{"key": "type", "params": "XCUIElementTypeMenuItem"}], "find": 99999})
            cut = next((item for item in items if str(item.get("label") or "") in ("剪切", "Cut")), None)
            if cut is None:
                # 菜单未出现：先单击确保聚焦，再补一次双击选择。
                self.tap(x, y)
                time.sleep(0.4)
                self.double_tap(x, y)
                time.sleep(0.8)
                ...
                cut = next((item for item in items if str(item.get("label") or "") in ("剪切", "Cut")), None)
            if cut is None:
                # 二级手势路径：长按进入选择模式，经「选择」菜单到达剪切。
                self.long_press(x, y, duration=0.6)
                time.sleep(0.8)
                items = self.find_elements({"sel": [{"key": "type", "params": "XCUIElementTypeMenuItem"}], "find": 99999})
                select = next((item for item in items if str(item.get("label") or "") in ("选择", "Select")), None)
                if select is not None:
                    self.tap(select["x"] + select["width"] / 2, select["y"] + select["height"] / 2)
                    time.sleep(0.6)
                    items = self.find_elements({"sel": [{"key": "type", "params": "XCUIElementTypeMenuItem"}], "find": 99999})
                cut = next((item for item in items if str(item.get("label") or "") in ("剪切", "Cut")), None)
            current = None
            if cut is not None:
                missing_menus = 0
                self.tap(cut["x"] + cut["width"] / 2, cut["y"] + cut["height"] / 2)
                time.sleep(0.5)
                failed_reads = 0
            else:
                missing_menus += 1
                if missing_menus >= 6: return False
            try:
                current = read_value()
                failed_reads = 0
            except (DeviceOperationError, DeviceResponseError, DeviceConnectionError):
            ...
            if current is not None:
                if not current.strip(): return True
                if previous is not None and len(current) >= len(previous):
                    stale_rounds += 1
                    if stale_rounds >= 3: return False
                else:
                    stale_rounds = 0
                previous = current
            elif failed_reads >= 5:
                return False
            if log: print(t("clear_text_cycle", cycle=cycle, length=len(current) if current is not None else -1, cut="yes" if cut is not None else "no-menu"))
        if log: print(t("clear_text_failed", max_cycles=max_cycles))
        try:
            return not read_value().strip()
        except (DeviceOperationError, DeviceResponseError, DeviceConnectionError):
            return False
