...
$('screen').addEventListener('click',e=>{if(cropFinished){cropFinished=false;return}let action=actionSpace(),tree=treeSpace(),im=e.target,x=e.offsetX/im.clientWidth*action.width,y=e.offsetY/im.clientHeight*action.height,treeX=x/action.width*tree.width,treeY=y/action.height*tree.height,best=null;$('coordinate').textContent=`Action coordinate: x=${x.toFixed(1)}, y=${y.toFixed(1)}`;for(const n of nodes){let r=rect(n);if(treeX>=r.left&&treeX<=r.right&&treeY>=r.top&&treeY<=r.bottom)best=n}if(best)pick(best._key)});
function toggleCrop(){cropMode=!cropMode;$('crop').textContent=cropMode?'Drag on screenshot':'Crop save';$('crop').style.background=cropMode?'#ffd166':'';}
