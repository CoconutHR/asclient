...
$('screen').addEventListener('click',e=>{let s=space(),im=e.target,x=e.offsetX/im.clientWidth*s.width,y=e.offsetY/im.clientHeight*s.height,best=null;$('coordinate').textContent=`Device coordinate: x=${x.toFixed(1)}, y=${y.toFixed(1)}`;for(const n of nodes){let r=rect(n);if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)best=n}if(best)pick(best._key)});
        ...
        def do_GET(self) -> None:
            ...
            if path == "/api/snapshot":
                try:
                    ...
                    width = int(display.get("widthPixels") or 0)
                    height = int(display.get("heightPixels") or 0)
                    if len(image) >= 24 and image.startswith(b"\x89PNG\r\n\x1a\n"):
                        width = width or int.from_bytes(image[16:20], "big")
                        height = height or int.from_bytes(image[20:24], "big")
                    data = json.dumps({"tree": tree, "app": app, "app_error": app_error, "coordinate_space": {"width": width or 1, "height": height or 1}, "image": base64.b64encode(image).decode("ascii")}, ensure_ascii=False).encode()
