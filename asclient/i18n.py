...
_MESSAGES: dict[str, tuple[str, str]] = {
    ...
    "cannot_reach_logs": ("无法连接 AScript 日志服务 {host}:{port}: {detail}", "cannot reach AScript log service at {host}:{port}: {detail}"),
    ...
    "init_loopback_hint": ("device.address 已设为 {address}（USB 隧道场景）。若使用 Wi-Fi 直连，请改为手机 AScript 页面显示的地址，例如 192.168.3.17:9096；否则 doctor 会因缺少 iproxy 报错。", "device.address was set to {address} for USB tunnel use. On direct Wi-Fi, change it to the address shown in the AScript app, for example 192.168.3.17:9096; otherwise doctor reports a missing-iproxy error."),
    "init_gitignore_warning": ("警告：{path} 位于 Git 仓库中且未被忽略。该文件将保存密码、UDID 与内网地址，请把 asclient.json 加入 .gitignore 后再填写。", "warning: {path} sits inside a Git repository and is not ignored. This file will hold a password, UDID and intranet address; add asclient.json to .gitignore before filling it in."),
