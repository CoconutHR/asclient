...
`tunnel` 在前台运行，按 `Ctrl+C`、向 ASClient 进程发送 `SIGTERM` 或发生命令异常时，都会进入清理路径并终止由 ASClient 启动的两个 `iproxy` 进程。客户端会在停止后等待本机转发端口重新可绑定，避免紧接着重启隧道时出现短暂端口冲突。不要把这些进程作为后台孤儿任务长期保留。
