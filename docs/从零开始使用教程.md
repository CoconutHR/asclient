...
`doctor` 成功时输出类似下面这样（未安装 `iproxy` 时第一行会是 `[错误] 未找到 iproxy…`，纯 Wi-Fi 使用可以忽略它；只有第 5 节的 USB 隧道需要 `iproxy`）：

```text
ASClient 环境诊断
[错误] 未找到 iproxy。无法自动安装第三方二进制；请安装受信任的 libimobiledevice，或用 doctor --fix-iproxy <路径> 写入已有 iproxy 的绝对路径。
[正常] 设备服务可访问，平台：iOS
...
不要关闭终端 A；按 `Ctrl+C` 会同时停止两条转发。默认转发 `10102` 日志端口，因此不要随意使用 `--no-logs`，否则 `log` 和 `deploy --logs` 无法通过 USB 工作。完整的多设备、端口调整和运维说明见 [USB 隧道运维指南](USB隧道运维指南.md)。
...
def main() -> None:
    ...
    with Run(device, artifacts_root="artifacts") as run:
        ...
        submit = run.assert_unique(device.selector().text(LOGIN_BUTTON))

        # set_text 会先点击控件取得焦点再输入。
        run.step("fill_username", lambda: username.set_text("demo@example.com"))
        run.step("fill_password", lambda: password.set_text("demo-password"))

        # 点击后等待首页标识出现；超时会自动采集失败证据。
...
- 步骤名建议用英文等 ASCII 字符：证据文件名由步骤名生成，当前版本的中文步骤名会使文件名退化为 `step_*`（`manifest.json` 中仍保留原始名称）；
...
- 控件等待的轮询间隔固定为最多 0.3 秒，由客户端控制，不要在等待 API 外再叠加 `time.sleep()`。
...
4. 先用 `py -m asclient help` 查看命令速查，再执行任何会改变手机状态的命令；`doctor`、`tunnel`、`deploy`、`log`、`tap` 等常用命令支持 `help <命令>` 查看详细参数，全部命令的完整参数见 [API 使用参考](API使用参考.md)的 CLI 一节。
