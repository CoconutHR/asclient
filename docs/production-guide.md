...
长列表中可使用 `scroll_until_image(template, confidence=0.95, max_swipes=8)`。它会先检查当前页面，再从下向上滑动并重试；找到后返回匹配结果但不自动点击。必须同时设置业务上合理的 `timeout` 和 `max_swipes`，避免页面滚动到末尾后无限重试。
