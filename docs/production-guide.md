...
`doctor` 发现 `error` 检查项时返回退出码 `1`，其余情况返回 `0`，可用于发布脚本。使用 `127.0.0.1` / `localhost` 且控制服务已可访问时，被占用的配置端口会识别为“活动 USB 隧道”并标为正常；在其他情况下端口占用仍显示为 `warning`。`--report` 输出的 JSON 含 `created_at`、`device` 和 `checks`；每个检查项有 `name`、`status`、`message` 与可选 `detail`，不含 `device.password`。
...
图标无法暴露控件树时，可使用本机模板等待：`wait_image(template, confidence=0.95, timeout=15, log=True)`，返回实际像素坐标的匹配结果；`wait_image_gone()` 用于等待加载图标消失，`tap_image()` 则等待后点击中心点。置信度必须在 `(0, 1]`，越高越严格。所有轮询等待（图像出现/消失、控件出现/消失、滚动找图）都有 `log=False`，设为 `True` 会在本机终端逐轮输出状态。图像等待默认先等待一个 `interval` 再首轮探测；要立即探测可设 `initial_delay=False`，首次等待仍计入总 `timeout`。模板应从同一分辨率、方向和 UI 缩放状态的真机截图中截取，并优先用 `region=(left, top, right, bottom)` 限制搜索区域以降低误匹配和轮询时间。
...
列表的滚动轨迹或速度不适配默认值时，使用 `swipe_relative=(x1_ratio, y1_ratio, x2_ratio, y2_ratio)` 和 `duration_ms`。该元组会覆盖 `direction`；例如 `swipe_relative=(0.7, 0.75, 0.35, 0.25), duration_ms=650`。
