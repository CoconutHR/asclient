...
`init` 生成完整配置且不连接设备；已存在同名文件时会拒绝覆盖并返回退出码 `1`，因此可安全地放进幂等的部署脚本（需要重建时显式加 `--force`）。多环境场景用 `--config` 分别生成独立配置，例如 `py -m asclient --config envs\device-a.json init --device 192.168.3.25:9096`。CI 中若需审查生成内容而不落盘，使用 `init --print`。

编辑 `asclient.json` 中的 `language`、`device.address`、`device.password`、`device.timeout` 和 `device.retries`。`language` 默认为 `auto`，中文系统输出中文，其他系统输出英文；可用 `--lang zh-CN` 或 `--lang en` 临时覆盖。真实配置已被 `.gitignore` 排除，不应提交——`init` 在检测到配置位于 Git 仓库内且未被忽略时会向标准错误打印警告。单次临时覆盖可使用 `--device`、`--password` 或 `--timeout`；这四个都是全局参数，必须写在子命令之前，覆盖优先级为“命令行 > 配置文件 > 内置默认值”。`--config` 可指向另一份配置文件，便于为不同设备或环境维护独立配置而不互相污染。

使用 `py -m asclient`（Windows；macOS/Linux 为 `python3 -m asclient`），不要依赖 Windows `Scripts` 目录是否已加入 `PATH`。`asc` 命令仅在该目录已加入 `PATH` 时可用。
...
`asclient` 发布包包含内联类型注解、中文 API docstring 与 `py.typed` 标记。PyCharm 和 VS Code 的 Pylance 均会据此提示方法签名、参数类型、默认值、返回类型以及中文用途说明。参数名保持英文，例如 `timeout`、`interval`、`duration`；前三者均以秒为单位，只有带 `_ms` 后缀的兼容参数使用毫秒。若刚升级本地源码，请在 IDE 解释器中重新执行 `py -m pip install --user --upgrade .`，再重启语言服务或重新打开项目。
...
真机 USB 验证（iPhone / iOS 26.6.1 / AScript 4001）中，HID JPEG 获取及 Pillow 解码中位约 `81ms`，完整 PNG 截图回传中位约 `406ms`；稳定热态的实际模板查询约 `73–126ms`，此前 PNG 路径约 `200–250ms`。首次匹配会叠加 HID 初始化和动作坐标校准，不能作为后续轮询基线。这些数值只代表该设备、模板和 USB 环境，生产验收仍应记录目标设备、iOS/AScript 版本、模板阈值与区域。
...
6. 需要取得局部模板时，点击顶部“框选区域”。Inspector 会立即冻结一张原始 PNG 并暂停实时刷新；拖动只形成或更新选区，**不会在松开鼠标时保存**。选区实时显示物理像素矩形、尺寸、中心点和 `region_relative` 比例坐标；确认后点击“保存 PNG”或按 `Enter`，服务端会从该冻结 PNG 精确裁剪，保存无损 PNG 和同名 JSON 元数据（含源截图尺寸、坐标、比例、前台 App 和采集时间）。按 `Esc` 或“取消”放弃选区。文件保存至启动 `py -m asclient inspect` 时的当前目录；终端目录路径即为保存位置。
...
```bat
py -m asclient --device 127.0.0.1:9096 inspect --port 8765 --no-browser
```
...
5. USB 连接使用 `AScriptTunnel` 或 `py -m asclient tunnel`，并只配置回环地址。默认同时映射 HTTP `9096` 和日志回显 `10102`；本机监听地址被强制限制为回环地址，无法通过参数绑定到局域网网卡。详细的设备配对、UDID、端口和防火墙要求见 [USB 隧道运维指南](USB隧道运维指南.md)。
...
| 现象 | 常见原因 | 处理 |
| --- | --- | --- |
| `无法连接 AScript 设备` / `cannot reach AScript device` | 地址、网络、端口或服务未启动 | 检查 iPhone IP、Wi-Fi 隔离、服务状态；先跑 `ping` |
...
**根因（服务端，已实锤）**：AScript 4001 IPA 内置服务的 `ascript/ios/developer/api/api_status.py` 第 49 行把实例属性当方法调用：
...
**客户端补偿（asclient 0.8.1 起）**：检测到该降级后，客户端自动用一次设备端 `eval` 读取等价的只读字段并回填到 `status` 结果中：

- 回填：`device`（型号信息）、`system`（iOS 版本、`language`、时区）、`app`（AScript 版本/build）、`python.version`；
...
若 AScript 新版 IPA 修复了 `/api/status`，接口正常返回，补偿逻辑不会触发。
